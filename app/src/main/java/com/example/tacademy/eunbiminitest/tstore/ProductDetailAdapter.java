...
public class ProductDetailAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getItemViewType(int position) {
        ...
        position--;
        if(product.getPreviewUrilist().size() > 0){
            if(position == 0) return VIEW_TYPE_TITLE;
            position --;
            if(position < product.getPreviewUrilist().size()){
                return  VIEW_TYPE_PREVIEW;
            }
            position -= product.getPreviewUrilist().size();
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if(product.getPreviewUrilist().size() > 0){
            ...
            if(position < product.getPreviewUrilist().size()){
                ...
                h.setPreViewImage(product.getPreviewUrilist().get(position));
                return;
            }
            position -= product.getPreviewUrilist().size();
        }
    ...
    public int getItemCount() {
        ...
        if(product.getPreviewUrilist().size() >0){
            size +=1;
            size += product.getPreviewUrilist().size();
        }
        if(product.getModels().modelList.size() > 0){
            size += 1;
